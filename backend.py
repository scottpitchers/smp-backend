...
for file in [USERS_FILE, PLAYERS_FILE, CONTENT_FILE, SCHEDULES_FILE, PAIRING_FILE, ANALYTICS_FILE]:
    if not file.exists():
...
def health():
    players = load_data(PLAYERS_FILE)
    active = sum(1 for p in players.values() 
                 if (datetime.utcnow() - datetime.fromisoformat(p.get('last_seen', datetime.utcnow().isoformat()))) 
